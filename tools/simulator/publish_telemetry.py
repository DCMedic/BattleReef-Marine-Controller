...
MQTT_CLIENT_ID = os.getenv("MQTT_CLIENT_ID", "simulator_node")
MQTT_IDENTITY = os.getenv("MQTT_IDENTITY", "simulator_node")
MQTT_CA_CERT = os.getenv("MQTT_CA_CERT", "/run/battlereef-mqtt/ca.crt")
MQTT_CLIENT_CERT = os.getenv("MQTT_CLIENT_CERT", "/run/battlereef-mqtt/simulator_node.crt")
MQTT_CLIENT_KEY = os.getenv("MQTT_CLIENT_KEY", "/run/battlereef-mqtt/simulator_node.key")
...
def connect_with_retry(client: mqtt.Client) -> None:
    while True:
        try:
            print(f"[SIM] Attempting authenticated MQTT connection to {MQTT_HOST}:{MQTT_PORT}")
            client.connect(MQTT_HOST, MQTT_PORT, 60)
            print(f"[SIM] Connected as certificate identity {MQTT_IDENTITY}")
            return
...
def publish_reading(
    ...
    now = utc_now()
    topic = f"{TOPIC_BASE}/{SOURCE_NODE}/{sensor_key}"
    payload = {
...
def leak_state(probability_wet: float = 0.003) -> float:
    return 1.0 if random.random() < probability_wet else 0.0
...
def main() -> None:
    ...
    try:
        while True:
            ...
            power_draw = bounded(
                420 + daylight * 110 + (return_flow - 800) * 0.12 + (manifold_flow - 300) * 0.08 + random.uniform(-6, 6),
                250,
                700,
                1,
            )
